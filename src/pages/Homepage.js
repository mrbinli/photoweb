...
const Homepage = () => {
  ...
  const initialURL = "/v1/curated?page=1&per_page=20";
  let searchURL = `/v1/search?query=${input}&per_page=15&per_page=1`;
  ...
  const morePiture = async () => {
    ...
    setPage(page + 1);
    if (currentSearch === "") {
      newURL = `/v1/curated?page=${page + 1}&per_page=15`;
    } else {
      newURL = `/v1/search?query=${currentSearch}&per_page=15&per_page=1`;
    }
